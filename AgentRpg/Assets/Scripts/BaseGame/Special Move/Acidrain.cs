...
public class Acidrain : GenericMove
{
    ...
    public override void ActivateMove()
    {
        ...
        for (int x = -1; x < 2; x++)
        {
            for (int y = -1; y < 2; y++)
            {
                ...
                {
                    Gridinfo.AllGrids[(int)Character_Info.LocationAction.y + y][(int)Character_Info.LocationAction.x + x].GetComponent<GridControl>().StatusIndex = 7;
                }
            }
        }
