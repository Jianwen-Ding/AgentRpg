        ...
        if (HasUsedCharge == true && MoveDecison.IsDisplayingHappening == true)
        {
            ...
            for (int x = 0; x < AreaCheck((int)Character_Info.LocationAction.x - 1, (int)Character_Info.LocationAction.y, 3, 1).Length; x++)
            {
                ...
                if (AreaCharacters[x] != null)
                {
                    ...
                    {
                        CheckedCharacterBase.Health -= (ChargestrikeExponentialChange * Character_Info.ExpressedDamage) * (ChargestrikeExponentialChange * Character_Info.ExpressedDamage);
                        ...
                        InWorldText.GetComponent<FadeOutText>().BeginInitiate(1, "" + (ChargestrikeExponentialChange * Character_Info.ExpressedDamage) * (ChargestrikeExponentialChange * Character_Info.ExpressedDamage), Color.black, new Vector2(5, 5));
                    }
                }
            }
            AreaEffect((int)Character_Info.LocationAction.x, (int)Character_Info.LocationAction.y, 1, 1);
